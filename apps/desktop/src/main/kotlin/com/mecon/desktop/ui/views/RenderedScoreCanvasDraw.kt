...
internal fun DrawScope.drawRenderedScore(request: RenderedScoreCanvasDrawRequest) {
    ...
    val beamPreview = beamDrag?.takeUnless { beamCommittedFrameDisplayed }?.let { drag ->
        selectedBeamControls
            ...
            ?.let { controls -> createBeamDragPreview(rr, controls, drag) }
    }
    val curvePreview = curveDrag
        ?.takeUnless { curveCommittedFrameDisplayed }
        ?.let { createCurveDragPreview(rr, it) }
    ...
    // Drag-to-transpose preview: the moved notes re-engraved at their new pitch (the
                // originals are hidden by the style track). Two layers — the unmoved parts of a
    // partially-moved chord in black (base), the dragged notes in the selection colour
        // on top (moved). Same global→page mapping as the ghost/playhead in paginated mode.
    ...
    selectedBeamControls?.let { controls ->
        ...
        for (point in listOf(
            ...
            val designPoint = if (paginatedView) {
                globalToDesign(point.x.value, point.y.value, pages, pageSlots)
            } else {
                Offset(point.x.value, point.y.value)
            } ?: continue
    ...
    if (selectedVoltaEndpoint != null && selectedVoltaElements.isNotEmpty()) {
        ...
        val targetBounds = targetMeasure?.let { measure ->
            rr.measureBounds.firstOrNull { it.measureNumber == measure }
        }
        ...
        val point = if (targetBounds == null) settled else {
            ...
            rr.transformerSnapshot.toAbsolute(
                RelativePoint(targetX, rr.transformerSnapshot.toRelative(settled).y)
            )
        }
        val designPoint = if (paginatedView) {
            globalToDesign(point.x.value, point.y.value, pages, pageSlots)
    ...
    val navElement = navDrag?.elementId?.let(rr::elementById)
        ?: selectedNavigationElements.firstOrNull()
    if (navDrag != null && navElement != null) {
        val dx = rr.transformerSnapshot.toPixels(
            StaffSpace(navDrag.previewDx)
        ).value
        val dy = rr.transformerSnapshot.toPixels(
    ...
    if (drag != null) {
        fun movedPoint(
            ...
            val oldAnchorX = rr.timeCodePositions[oldTime]?.x ?: original.x.value
            val newAnchorX = rr.timeCodePositions[newTime]?.x ?: oldAnchorX
            val dxPx = newAnchorX - oldAnchorX + rr.transformerSnapshot
                .toPixels(StaffSpace(newDx - oldDx)).value
            val dyPx = rr.transformerSnapshot.toPixels(StaffSpace(newDy - oldDy)).value
        ...
        rr.elementById(drag.elementId)?.let { element ->
    ...
    if (attachmentSection != null && selectedAttachmentElements.isNotEmpty()) {
        ...
        val times = buildList {
            ...
            when (attachment) {
                is com.mecon.api.computed.ComputedHairpin -> add(drag?.endTime ?: attachment.endTime)
                is com.mecon.api.computed.ComputedOctaveShift -> add(drag?.endTime ?: attachment.endTime)
                is com.mecon.api.computed.ComputedOrnamentMark -> attachment.endTime?.let {
                    add(drag?.endTime ?: it)
                }
                is com.mecon.api.computed.ComputedTempoKeyframe -> if (attachment.isGradual) {
        ...
        for ((index, time) in times.withIndex()) {
            ...
            val anchor = if (attachment is com.mecon.api.computed.ComputedBreathMark) {
                breathBoundaryAnchor(
                    rr, time, attachment.staffIndex, owningSystemIndex, symbol,
                )
            } else {
