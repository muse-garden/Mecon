...
/** A snapped time-signature preview: target measure plus absolute render commands. */
data class GhostTimeSignature(
    val measure: Int,
    val onset: TimeCode,
...
/**
 * Ghost preview for the time-signature pen. Mirrors [GhostClefComputer] but snaps to the nearest
 * **measure downbeat** (a time signature always governs a whole measure) and previews the meter
 * glyph just right of that barline. The resolved [GhostTimeSignature.measure] is what the edit
 * applies to.
 */
context(BravuraFont)
class GhostTimeSignatureComputer(private val config: RenderLayoutConfig = RenderLayoutConfig.DEFAULT) {
    fun compute(
        result: RenderResult,
        ...
        runtime.orderedStaffs().getOrNull(staffHit.staffIndex) ?: return null
        ...
        val element = TimeSignatureElement.create(
            time = onset,
    ...
    /** Nearest measure downbeat (as a [TimeCode]) to [targetAbsX] on the system at [systemY]. */
