...
4. **几何 + 符头列对齐**：用 `NoteBodyElementBuilder` + `VoiceEventLayoutBuilder` + `StemDirectionResolver`（按当前声部取默认符干方向）生成 `RenderCommand` 列表（绝对坐标），休止符则只画休止符字形。`timeCodePositions.x` 是整个 slot 的**右边缘**，不能当作已有音符的符头边缘：临时记号、附点、和弦二度错位和同拍多声部避让都可能把符头推到 slot 内更靠左的位置。`RenderResultAssembler` 因此从最终 `NoteElement` 布局生成按 `time + staffIndex + voiceNumber` 索引的 `noteheadRightPositions`，并在同一次全谱遍历中预计算每个 onset 的“跨谱表同声部号共享列”和“全局共享列”；ghost 按**本谱表本声部 → 全谱同声部号 → 全谱任意声部 → slot X**回退。钢琴左手同拍为空时也会对齐右手已有符头，而 hover 热路径只做常数时间查表，不扫描全谱。ghost 自身仍整体左移到“符头右缘贴锚点”（`drawOffset.x = anchorRelX − 符头右伸量`），所以临时记号、附点、articulation / ornament 等附件、符杠与跨声部碰撞都不会把预览误锚到附件包围盒。对齐时间轴布局继续使用共享 boundary + `notationContentStartGap`，不套用普通谱面的右缘规则。`anchor` 随同偏移（分页时定位所属页）。返回 `GhostNote(voiceTrackId, staffTrackId, voiceNumber, onset, pitch, commands, anchor)`。
