...
所选音符的最早 onset，或所选和声时间轴和弦框；新区域默认延伸到谱尾，但若后方存在实际调号
变更，则在该变更处截止。弹层复用
...
吸附，避免续行片段被误当作整个调性区间的端点。分页横向拖动在端点原行、原页面附近锁定其源
系统；只有指针真正进入另一系统的整体排版范围才切换目标行，在行间空白继续保持当前目标，不以
最近五线谱核心、调性条中线或行间中点推断下一行；横向双页布局中进入另一页面会立即解除原行锁定，
并用页面横向范围区分显示高度相同的系统。蓝色端点预览的 Y 固定到目标系统的同一注释 lane：
优先对齐该系统已有片段，否则保留源端点相对五线谱的垂直偏移。悬停或拖动端点时使用水平双箭头光标。
...
小调内的音级按同调号的关系大调编号显示（例如 a 小调为 `6, 7, 1, 2, 3, 4, 5`）；谱面标签、
单音选择与多音候选都委托 `ModulationPitchLabels`，不在面板里换算。选中已有调性区域时，右栏只
显示调名、结束后的调性中心和编辑按钮；编辑按钮打开与插入入口共用的调性选择弹层。
弹层打开后可继续在谱面选择音符以刷新单音音级/多音候选；音符选择不会清除当前调性区域编辑目标，
点击其他注释或清空选择才结束该区域的编辑状态。
